...
class AssetImportValidator
{
    ...
    private function validateRequiredFields(TempAsset $row): array
    {
        ...
        // Relaxed validation as donnees.csv is missing some data
        /*
        ...
        */
        ...
        /*
        if (empty($row->asset_number) && empty($row->cab_number)) {
            $errors[] = [
                'field' => 'asset_number',
                'message' => 'Asset number or CAB number is required',
                'code' => 'REQUIRED_FIELD',
            ];
        }
        */
        ...
        if (empty($row->designation)) {
            // Use CAB number or asset number as fallback designation
            $row->designation = $row->cab_number ?? $row->asset_number ?? 'Inconnu';

            // Also update the property so we don't save an empty value
            $row->save();
        }
    ...
    private function validateDuplicates(TempAsset $row): array
    {
        ...
        if (!empty($sn)) {
            $row->serial_number = $sn; // update the model so it gets saved clean

            // Check in existing assets
            if (isset($this->existingSerialNumbers[$sn])) {
                // If it exists, we'll append something to avoid dropping the row or make it null?
                // Append -dup to save
                $sn = $sn . '-dup-' . uniqid();
            ...
            elseif (isset($this->batchSerialNumbers[$sn]) &&
                    $this->batchSerialNumbers[$sn] !== $row->id) {
                // Append -dup to save
                $sn = $sn . '-dup-' . uniqid();
                $row->serial_number = $sn;
                $row->save();
    ...
    public function moveValidRowsToAssets(int $batchId): int
    {
        ...
        TempAsset::where('import_batch_id', $batchId)
            ->where('validation_status', 'valid')
            ->chunkById(500, function ($rows) use (&$movedCount) {
                foreach ($rows as $tempAsset) {
                    try {
                        ...
                        DB::transaction(function () use ($tempAsset, &$movedCount) {
                            ...
                            // Clean serial number: if empty string, make it null
                            $serialNumber = trim((string)$tempAsset->serial_number) === '' ? null : $tempAsset->serial_number;
